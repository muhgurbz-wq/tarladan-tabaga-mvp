...
const STATE = {
  screen:'overview', category:'all', query:'', selected:'domates', qty:5, cart:[], payment:null, order:null, orderStage:0, autoRun:true,
...
function startOperation(){ if(!STATE.payment||!STATE.cart.length)return; buildMissions(); STATE.order={id:'TT-'+Date.now().toString().slice(-8),createdAt:Date.now(),total:STATE.payment.total}; STATE.orderStage=0; STATE.autoRun=true; renderOperations(); switchScreen('operations'); startAutoTimer(); }
function advanceOrder(){ if(!STATE.order||STATE.orderStage>=ORDER_STAGES.length-1)return; STATE.orderStage++; if(STATE.orderStage>=5)STATE.missions.forEach(m=>m.status=STATE.orderStage===5?'Görev hazır':STATE.orderStage===6?'Uçuşta':'Tamamlandı'); renderOperations(); updateOverviewFlow(); }
function startAutoTimer(){ if(STATE.timer)clearInterval(STATE.timer); if(!STATE.autoRun||!STATE.order)return; STATE.timer=setInterval(()=>{ if(STATE.orderStage===3){STATE.autoRun=false;clearInterval(STATE.timer);STATE.timer=null;renderFlow();return;} if(STATE.orderStage<ORDER_STAGES.length-1)advanceOrder(); else {clearInterval(STATE.timer);STATE.timer=null;} },4500); }
function toggleAuto(){ STATE.autoRun=!STATE.autoRun; if(STATE.autoRun)startAutoTimer(); else if(STATE.timer){clearInterval(STATE.timer);STATE.timer=null;} renderFlow(); }

function renderOperations(){ renderFlow(); renderMissions(); renderSettlement(); if(STATE.opsPane==='live')setTimeout(initMap,80); }
function renderFlow(){
  $('#orderId').textContent=STATE.order?.id||'—'; $('#orderProgress').style.width=STATE.order?`${((STATE.orderStage+1)/ORDER_STAGES.length)*100}%`:'0%'; $('#currentStage').textContent=STATE.order?ORDER_STAGES[STATE.orderStage][0]:'Görev bekleniyor'; $('#currentStageText').textContent=STATE.order?ORDER_STAGES[STATE.orderStage][1]:'Sepetten güvenli ödeme oluşturup operasyonu başlatın.'; const action=$('#stageAction'),auto=$('#toggleAuto'); action.disabled=!STATE.order||STATE.orderStage>=ORDER_STAGES.length-1; auto.disabled=!STATE.order; action.textContent=STATE.orderStage===3?'Fotoğrafları onayla':STATE.orderStage===4?'Paketlemeyi doğrula':'Sonraki adım'; auto.textContent=`Otomatik: ${STATE.autoRun?'açık':'kapalı'}`;
...
function runSelfTest(){
  ...
  t('48 ürün kataloğu',PRODUCTS.length===48); t('5 ana kategori',new Set(PRODUCTS.map(p=>p.category)).size===5); t('5 ana navigasyon',$$('.bottomnav [data-nav]').length===5); t('ürün pasaportu',!!$('#passport')&&$$('#passportTabs button').length===6); t('çoklu üretici görev motoru',typeof buildMissions==='function'); t('güvenli ödeme simülasyonu',typeof createPayment==='function'); t('bölgesel koordinat haritası',Object.keys(CITY_COORDS).length>=20); t('resmi fiyat veri yükleyici',typeof loadPrices==='function'); t('drone canlı rota katmanı',typeof initMap==='function'); t('jüri demo modu',typeof juryDemo==='function'); t('manuel jüri kontrolü',typeof advanceOrder==='function'&&typeof toggleAuto==='function'); t('simülasyon etiketleri',document.body.textContent.includes('SİMÜLASYON')); t('tek CSS + tek JS',document.querySelectorAll('link[rel="stylesheet"][href*="competition.css"]').length===1&&document.querySelectorAll('script[src*="competition.js"]').length===1);
...
function bind(){
  ...
  $('#createPayment').onclick=createPayment; $('#startOperation').onclick=startOperation; $('#juryDemo').onclick=juryDemo; $('#stageAction').onclick=()=>{advanceOrder();if(STATE.autoRun)startAutoTimer();}; $('#toggleAuto').onclick=toggleAuto;
